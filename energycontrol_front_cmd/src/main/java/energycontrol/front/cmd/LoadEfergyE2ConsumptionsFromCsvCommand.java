...
public class LoadEfergyE2ConsumptionsFromCsvCommand extends Command 
{
	private ResourceBundleReader resourceBundleReader;
	private EnergyControlBackService backService;
	
	public LoadEfergyE2ConsumptionsFromCsvCommand(ResourceBundleReader resourceBundleReader, Scanner scanner, EnergyControlBackService backService) 
	{
		super(scanner);
	...
	public ActionResult execute() 
	{
		...
		else
		{
			...
			else
			{
				...
				//Cargar los consumos
				GenericActionResult<Source> garSource = this.backService.loadEfergyE2ConsumptionsFromCsv(fCsv.getAbsolutePath(), sourceDate);
